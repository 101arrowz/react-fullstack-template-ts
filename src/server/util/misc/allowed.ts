...
const accessibleData: { [k in LocalRestrictionLevel]: Restricter } = {
  ...
  async friends(data, requester, owner) {
    ...
    return (
      !!doc && !!doc.profile.friends && doc.profile.friends.includes(requester)
    );
  },
  ...
  async unset(data, requester, owner) {
    const doc = owner || (await userDB.resolve(data.owner));
    return !!doc && doc.prefs.private
      ? this.owner(data, requester, owner)
      : this.all(data, requester, owner);
  }
};
async function allowed<T extends Visible>(
  ...
  if (!newData.hasOwnProperty('owner') && owner) newData.owner = owner._id;
  if (!newData.owner) return;
  if (owner && owner._id !== newData.owner) owner = undefined;
  const isAllowed = await accessibleData[newData.restrictedTo || 'unset'](
    newData,
    requester,
    owner
  );
  if (isAllowed) {
    for (const kv of Object.entries(newData).filter(
      el => el instanceof Object && el.hasOwnProperty('restrictedTo')
    ) as [keyof typeof newData, Visible][]) {
      // Assume that all required properties will not have a restriction level
      const newValue = await allowed(
        {
          ...(owner && { owner: newData.owner || owner._id }),
          ...kv[1]
        } as Owned & Visible,
        requester,
        owner as User
